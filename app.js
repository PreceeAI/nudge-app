...
// iOS-style share icon: upward arrow leaving a box
const SHARE_BTN_SVG = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M8 10V1M8 1L5 4M8 1l3 3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><path d="M3.5 7H3a1 1 0 00-1 1v6a1 1 0 001 1h10a1 1 0 001-1V8a1 1 0 00-1-1h-.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>`;
...
function renderToday(content) {
  ...
  const shareBtn1 = canShare
    ? `<div class="today-card__actions"><button class="btn-share" data-card="1" aria-label="Share this card">${SHARE_BTN_SVG}</button></div>`
    : '';
  if (slot === 'morning') {
    card1.innerHTML = `
  ...
  // --- Card 2: Micro-action (morning/afternoon) or Gratitude nudge (night) ---
  if (slot === 'night') {
    document.getElementById('today-card-2').innerHTML = `
      <p class="today-card__label">Gratitude</p>
      <p class="gratitude-nudge">${slotData.gratitude_nudge}</p>
      ${canShare ? `<div class="today-card__actions"><button class="btn-share" data-card="2" aria-label="Share this card">${SHARE_BTN_SVG}</button></div>` : ''}
    `;
  } else {
    ...
    document.getElementById('today-card-2').innerHTML = `
      ...
      <div class="today-card__body">
        ...
        <div class="today-card__actions">
          ...
          ${canShare ? `<button class="btn-share" data-card="2" aria-label="Share this card">${SHARE_BTN_SVG}</button>` : ''}
        </div>
      </div>
    `;
    initDidThis();
  ...
  const shareBtn3 = canShare
    ? `<div class="today-card__actions"><button class="btn-share" data-card="3" aria-label="Share this card">${SHARE_BTN_SVG}</button></div>`
    : '';
  if (slot === 'morning') {
